...
- **Operator safety note.** The Phase 2.2a volume-mount-path edit is inert until somebody runs `up -d`. The branch must NOT be brought up via `./scripts/infra.sh up -d` until Phase 4a step 1 has migrated the on-disk layout — otherwise PG18 finds an empty `/var/lib/postgresql/18/docker` and either refuses to start or `initdb`s a fresh empty cluster while the PG16 data sits dormant at the volume root.
...
3. Re-run the row-count queries from Phase 0 step 5 — **two separate captures**, matching the split there. The strict diff is over the preserved file (must be empty); the outbox count is informational and may legitimately differ due to 3-day retention:
   ...
   ```
4. Confirm no application writes have happened. The application is not running and the connector is not registered, so the only possible writers are autovacuum/checkpointer (neither writes user-table rows) and any pg_cron jobs scheduled to fire on cluster start. Check:
   ```bash
   docker compose exec -T postgres psql -U user -d wealthpay -c \
     "SELECT jobid, schedule, command, last_run_status FROM cron.job_run_details
        ORDER BY end_time DESC NULLS LAST LIMIT 10;"
   ```
   If a pg_cron job ran since the cluster came up and that job mutates a table in the contract list (e.g. `outbox_cleanup_log`), capture and explain the count delta in the PR description; otherwise treat any count difference as a regression.
...
4. **Verify the four checkpointer metrics are present:**
   ```bash
...
6. **Confirm Prometheus is scraping cleanly:**
   ```bash
   curl -sG 'http://localhost:9090/api/v1/query' --data-urlencode 'query=up{job=~".*postgres.*"}' | jq '.data.result[] | {metric, value}'
   ```
   All `up` values must be `"1"`.
...
7. **Smoke test — open one account, check it shows up downstream:**
   ```bash
   curl -X POST http://localhost:8080/api/v1/accounts \
     -H 'Content-Type: application/json' \
     -d '{"ownerId":"00000000-0000-0000-0000-000000000001","openingBalance":{"amount":"100.00","currency":"USD"}}'
   sleep 2
   docker compose exec -T postgres psql -U user -d wealthpay -c \
     "SELECT account_id, balance FROM account.account_balance_view ORDER BY account_id DESC LIMIT 1;"
   ```
   The new account must be present in `account_balance_view` (which is populated by the Kafka outbox consumer, so this exercises the entire WAL → Debezium → Kafka → consumer → projection chain).
