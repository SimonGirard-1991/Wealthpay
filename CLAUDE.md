...
- **ArchUnit** (`architecture.HexagonalArchitectureTest`) — enforces hexagonal layering _within_ a BC, which Modulith does not cover. 16 rules grouped:
  ...
  - **`@Transactional` location**: only in `..application..` (use-case orchestration) or `..infrastructure.db..` — read-side `readOnly=true` repos, and `MANDATORY` where a multi-statement write must inherit the caller's boundary instead of opening one.
